...
from utils.mm.tokenization import BertTokenizer
from utils.mm.modeling import LXRTFeatureExtraction as VisualBertForLXRFeature, VISUAL_CONFIG
from utils.mm.modeling import VBFeatureExtraction
from utils.mm.modeling import UniterFeatureExtraction 
...
class VBEncoderBase(nn.Module):
    def __init__(self, args):
        ...
        self.model = VBFeatureExtraction.from_pretrained(
                "bert-base-uncased")
        if args.from_scratch:
            print("initializing all the weights")
            self.model.apply(self.model.init_bert_weights)
...
class VBEncoderLarge(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.max_seq_length = args.max_seq_length
        set_visual_config(args)
...
class UniterEncoderBase(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.max_seq_length = args.max_seq_length
        set_visual_config(args)
        ...
        self.model = UniterFeatureExtraction.from_pretrained(
                "bert-base-cased")
        if args.from_scratch:
            print("initializing all the weights")
            self.model.apply(self.model.init_bert_weights)
...
class UniterEncoderLarge(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.max_seq_length = args.max_seq_length
        set_visual_config(args)
